$(document).ready(function () {
		  ...
		  $.ajax({
			url: 'http://localhost:9000/carstatus',
			type: 'get',
			...
			success: function (res, textStatus, xhr) {
			  $.each(res.orders, function (index) {
				...
				'<tr><th scope ="row">' +  res.orders[index].cid.cname + '</th>' +
	'<td class="w-25">'+ "<img src='http://localhost:9000/images/" + res.orders[index].cid.carimg + "' class='img-fluid img-thumbnail'></td><td>" +
				  ...
				  "<Button class=' btn btn-success crate' btnrate= '" + res.orders[index]._id + "'>Rate</Button></br></td></tr>"
				);
			  });
			},
	...
	$("#logout").click(function(){
     ...
     error: function() {
       alert("Sorry, you are not logged in.");
		  ...
		  $('#cartable').on('click', '.crate', function () {
			...
			$.ajax({
			  url: 'http://localhost:9000/carrating',
			  type: 'post',
			  dataType: 'json',
			  data: data,
			  beforeSend: function (xhr) {
